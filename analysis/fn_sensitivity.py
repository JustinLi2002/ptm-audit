...
def run(arm, alphas, draws, n_bins, seed, out):
    ...
    by_alpha = (df.groupby("alpha")[["d_auroc", "d_auprc"]]
                  .mean().reset_index())

    print("\n=== mean over 8 tasks x 3 splits ===")
    print(f"{'alpha':>6s} {'dAUROC':>9s} {'dAUPRC':>9s} {'tasks<0':>8s}")
    for a in alphas:
        s = df[df.alpha == a].groupby("task")["d_auroc"].mean()
