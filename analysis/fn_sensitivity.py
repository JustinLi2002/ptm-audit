...
def load(results, arm):
    """Alpha-grid rows only.

    Runs with --inflations add rows whose alpha was solved per task; several
    of those solve to exactly zero where the target exceeds a task's ceiling,
    and they would otherwise be pooled into the alpha = 0 group, over-weighting
    precisely the low-ceiling tasks.
    """
    d = pd.read_csv(Path(results) / f"fn_sensitivity_{arm}.tsv", sep="\t")
    if "mode" in d.columns:
        return d[d["mode"] == "alpha"]
    if "target_inflation" in d.columns:
        return d[d["target_inflation"].isna()]
    return d
